...
| SDK Surface | Use Case | MCP Value | Risk | Shape | Proposed MCP Tool | Decision | Notes |
|---|---|---:|---:|---|---|---|---|
| `robot.events` + event subscriptions | Reactive workflows from robot events | High | Medium | Composed | `vector_wait_for_event` (bounded one-shot) | Now (bounded) | Implement one-shot wait with strict timeout/filter semantics; defer persistent subscribe/unsubscribe APIs. |
| `robot.events` persistent subscriptions | Long-lived reactive automation channels | Medium | High | Composed | `vector_event_subscribe` / `vector_event_unsubscribe` | Later | Defer until session lifecycle, cancellation, and resource-leak safeguards are proven. |
